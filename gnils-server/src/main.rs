...
fn start_server(mut commands: Commands) {
    ...
    let hash_hex: String = cert.certificate_chain().as_slice()[0]
        .hash()
        .as_ref()
        .iter()
        .map(|b| format!("{b:02x}"))
...
fn on_client_connected(
    ...
    if clients.len() >= 2 {
        info!("Extra connection from {peer:?} ignored");
        return;
    }
...
fn on_client_disconnected(
    ...
    if !matches!(
        phase.get(),
        ServerPhase::WaitingForShot | ServerPhase::SimulatingFlight | ServerPhase::RoundOver
    ) {
        return;
    }
...
fn receive_client_msgs(
    ...
    for (peer, msg) in incoming {
        match msg {
            ClientMsg::Ready => {
                if *phase.get() != ServerPhase::WaitingForReady {
                    continue;
                }
            ...
            ClientMsg::AimUpdate { angle, power } => {
                if *phase.get() != ServerPhase::WaitingForShot {
                    continue;
                }
                if Some(peer) != state.active_peer {
                    continue;
                }
                for (remote_id, mut s) in senders.iter_mut() {
                    if remote_id.0 != peer {
...
fn tick_round_setup(
    ...
    state.active_player = if state.round == 1 {
        1
...
fn launch_missile(state: &mut RoundState, angle: f64, power: f64, settings: &GameSettingsData) {
    ...
    let x = if state.active_player == 1 {
        PLAYER1_X
    } else {
        PLAYER2_X
    };
    let gun = if state.active_player == 1 {
        GUN_OFFSET_P1
    } else {
        GUN_OFFSET_P2
    };
    ...
    state.trail_color = if state.active_player == 1 {
        PLAYER1_COLOR
    } else {
        PLAYER2_COLOR
    };
...
fn tick_simulation(
    ...
    if let Some(col) = check_collisions(
        &state.missile,
        &planets,
        &player_y,
        active_player,
        ...
        match &col.kind {
            ColKind::Ship(_) => {
                ...
                let (hit, delta) =
                    resolve_hit(&col, active_player, last_power, &player_attempts);
                apply_scores(&mut state.scores, &hit, delta);
                ...
                if settings.0.particles_enabled {
                    ...
                    for (_, mut s) in senders.iter_mut() {
                        s.send::<Reliable>(ServerMsg::ParticleSpawn {
                            pos,
                            count: 30,
                            size: 10,
                        });
                ...
                for (_, mut s) in senders.iter_mut() {
                    s.send::<Reliable>(ServerMsg::RoundResult {
                        hit: hit.clone(),
                        scores,
                        game_over,
                    });
                ...
                next.set(if game_over {
                    ServerPhase::GameOver
                } else {
                    ServerPhase::RoundOver
                });
            }
            _ => {
                if settings.0.particles_enabled && matches!(&col.kind, ColKind::Planet) {
                    let pos = (col.pos.0 as f32, col.pos.1 as f32);
                    for (_, mut s) in senders.iter_mut() {
                        s.send::<Reliable>(ServerMsg::ParticleSpawn {
                            pos,
...
fn check_collisions(
    ...
    if m.flight < 0 && !is_on_screen(m.pos) {
        return Some(ColInfo {
            pos: m.pos,
            kind: ColKind::Miss,
        });
    }
    if !is_in_extended_range(m.pos) {
        return Some(ColInfo {
            pos: m.pos,
            kind: ColKind::Miss,
        });
    ...
    for (i, &y) in py.iter().enumerate() {
        ...
        if ship_id == active_player && m.flight > s.max_flight - SELF_HIT_GRACE_TICKS {
            continue;
        }
...
fn apply_scores(scores: &mut [i32; 2], hit: &HitResult, delta: i32) {
    if let HitResult::Ship { shooter, .. } = hit {
        scores[(*shooter - 1) as usize] += delta;
    }
}
