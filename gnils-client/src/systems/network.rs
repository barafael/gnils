...
impl Plugin for GnilsClientNetPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.add_systems(
            Update,
            receive_server_msgs.run_if(
                not(in_state(GamePhase::MainMenu)).and(not(in_state(GamePhase::Connecting))),
            ),
        );

        app.add_systems(
            Update,
            send_aim_update.run_if(in_state(GamePhase::Aiming)),
        );
...
fn start_connection(mut commands: Commands, join_addr: Res<JoinAddress>) {
    ...
    let entity = commands
        .spawn((
            ...
            WebTransportClientIo {
                certificate_digest: join_addr.cert_hash.clone(),
            },
        ))
        .id();
    ...
    for msg in msgs {
        match msg {
            ...
            ServerMsg::RoundSetup {
                ...
                for (_, mut marker, mut vis) in missile_q.iter_mut() {
                    marker.active = false;
            ...
            ServerMsg::OpponentAim { angle, power } => {
                ...
                for (mut player, _) in players.iter_mut() {
                    if player.id == opponent_id {
                        let initial_angle =
                            if player.id == 1 { 0.0 } else { std::f64::consts::PI };
                        player.angle = angle;
                        player.power = power;
            ...
            ServerMsg::MissileUpdate {
                ...
                // Ignore stale MissileUpdate that arrives (unreliable channel)
                // after a RoundResult has already ended the round.
                if turn.round_over {
                    continue;
                }
                for (mut body, mut marker, mut vis) in missile_q.iter_mut() {
                    let was_active = marker.active;
                    ...
                    if snapshot.active {
                        ...
                        *vis = Visibility::Visible;
                        if !was_active && *phase.get() == GamePhase::Aiming {
                            turn.firing = true;
